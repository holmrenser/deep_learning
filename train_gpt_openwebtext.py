...
DEVICE = torch.device(
    "cuda"
)  # 'mps' for ARM macbooks, 'cuda' for colab, 'cpu' otherwise
DATASET = "openwebtext"
...
class OWTDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        for d in self.dataset:
            ...
            if remainder.numel():
                chunk = torch.cat(
                    [remainder, tokens[: self.context_size - len(remainder) + 1]]
                )
...
if __name__ == "__main__":
    ...
    optimizer = torch.optim.AdamW(
        params=model.parameters(), lr=learning_rate, betas=(0.9, 0.99)
    )
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        optimizer, max_lr=0.01, total_steps=train_steps
    )
    model.train()
    ...
    for i, (train_x, train_y) in enumerate(tqdm(train_dataloader, total=train_steps)):
        ...
        # forward the model
        try:
            _, train_loss, train_accuracy = model(
